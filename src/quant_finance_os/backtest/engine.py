...
class BacktestEngine:
    ...
    def run(self, market_events: Iterable[MarketEvent]) -> BacktestResult:
        ...
        def normalize(event, parent_event_id: str | None = None):
            nonlocal seq
            seq += 1
        ...
        for raw_market_event in market_events:
            ...
            market_event = normalize(raw_market_event)
            ...
            for signal in signals:
                normalized_signal = normalize(signal, parent_event_id=market_event.metadata.event_id)
                ...
                normalized_order = normalize(order, parent_event_id=normalized_signal.metadata.event_id)
